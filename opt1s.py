...
class CustomEmbeddingLayer(nn.Module):
    def __init__(self, input_size, embedding_size):
        ...
        self.embedding = nn.Parameter(torch.ones(input_size, embedding_size))  # Initialize weights to 1
...
class Transformer(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_heads, output_size, num_gpu_batches):
        ...
        self.embedding_layer = CustomEmbeddingLayer(input_size, hidden_size)
    ...
    def forward(self, x):
        ...
        x = x.view(self.num_gpu_batches, -1, seq_length, input_size).transpose(0, 1)  # [num_batches, batch_size, seq_length, input_size]
...
def tokenize(prompt):
    return [1] * (32 * 768)  # Placeholder for tokenization result
...
with torch.no_grad():
    for tensor in tqdm(dataset_tensors, desc="Processing prompts"):
        ...
        output = transformer_model(torch.tensor(tensor).view(1, 32, 768))  # Add batch dimension
...
#opt = transformer_model(torch.ones(input_size))
